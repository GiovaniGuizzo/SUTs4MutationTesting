...
/**
 * This class implements an ordered set of integers; it supports the basic sets
 * operation: union as well as search, removal and insertion.
 *
 * Provided by the SQUALL Lab at Carleton University
 *
...
public class OrdSet {
    ...
    // ---------
    /**
     * Determines the size to set the array to from any given size. This method
     * ensures that the size of the array is at least equal to and always the
     * closest multiple of min_set_size to the proposed size n. If the proposed
     * size is greater than the max_set_size, the returned size is max_set_size.
    ...
    /**
     * Initializes the array. Sets all of the variables to their default. It
     * also copies the provided values into the ordered set up to the set size.
     *
    ...
    /**
     * Increments the size of the array, if possible, by min_set_size. If the
     * maximum size or the maximum resizes is reached, _overflow is set to true.
     */
    ...
    /**
     * Performs a binary search on the provided array for the element x. A
     * binary search is a search algorithm for finding a particular value in a
     * list of data. Binary search assumes the data is sorted. The search begins
     * by examining the value in the center of the list; because the values are
     * sorted, it then knows whether the value occurs before or after the center
     * value, and searches through the correct half in the same way.
     *
    ...
    // ---------
    /**
     * Create an empty instance of OrdSet with default values and a size based
     * on the proposed size.
     *
    ...
    /**
     * Create by copy from an array of values. The ordered set size will be
     * defined based on the length of the parameter array v; if the length of v
     * exceeds max_set_size, only the first values in the array up to
     * max_set_size will be considered and the set would have a size of
     * max_set_size
     *
     * @param v - The array of values to copy from into the ordered set.
     */
    ...
    /**
     * @return The ordered set as an array (note that it may include zeros for
     * the empty slots)
     */
     ...
     * @return 0 if the sets are equal (have the same elements, 1 if they are
     * the same size, but not equal, and the difference in size otherwise.
     */
    ...
    /**
     * Checks if this ordered set contains the ordered set given by x, all
     * elements in x are elements in this.
     *
    ...
    /**
     * Removes an element from the set. The element to remove is given by the
     * val parameter.
     *
    ...
    /**
     * Adds an item to the set. Since this is a set, the element is only added
     * if it is unique. An attempt to add a new element while the set is full
     * causes a set resize. When no resizes are allowed, an overflow exception
     * is raised.
     *
     ...
     * @return The ordered set element at the given position or -1 if the item
     * is out of bound.
     */
    ...
    /**
     * Makes a free slot in the array, to place the value n into. This free slot
     * will be made where it is appropriate to store the value n and maintain
